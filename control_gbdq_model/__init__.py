...
from gym_PBN.envs.pbn_control_multi import PBNControlMultiEnv
...
class ControlGBDQ(nn.Module):

    def __init__(self, observation, config, env: PBNControlMultiEnv):
        super().__init__()
        ...
        self.action_count = len(env.control_nodes)

        self.q = GraphBranchingQNetwork(observation, 2, self.action_count).to(device=config.device)
        self.target = GraphBranchingQNetwork(observation, 2, self.action_count).to(device=config.device)
    ...
    def predict(self, state, target):
        with torch.no_grad():
            ...
            if np.random.random() < epsilon:
                action = np.random.randint(0, 1, size=self.action_count)
